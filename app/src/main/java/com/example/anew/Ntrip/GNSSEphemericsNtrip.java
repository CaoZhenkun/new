...
//        }
    ...
    public SatellitePosition getSatPositionAndVelocities(long unixTime, double range, int satID, char satType, double receiverClockError)
    {
        ...
        if(true) {
            //这里给一个PPP和SPP的判断语句
             satellitePosition = computeSatPositionAndVelocities(unixTime, range, satID, satType, ephGps, receiverClockError);
        }
        //else if(true){
             //satellitePosition = computeSatPositionAndVelocitiesPPP(unixTime, range, satID, satType, ephGps, receiverClockError);
        //}
