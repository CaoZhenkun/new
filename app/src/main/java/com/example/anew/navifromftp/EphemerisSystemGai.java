...
public abstract class EphemerisSystemGai {
    ...
    public SatellitePosition computeSatPositionAndVelocities(long unixTime, double obsPseudorange,int satID, char satType, EphGps eph, double receiverClockError) {
        ...
        double MU_GPS   =3.9860050E14;     //gravitational constant
        double MU_GAL   =3.986004418E14;   //earth gravitational constant
        double MU_BDS   =3.986004418E14;   //earth gravitational constant
        double OMGE_GPS=7.2921151467e-5;   // earth angular velocity (rad/s)
        double OMGE_GAL =7.2921151467E-5;  // earth angular velocity (rad/s)
        double OMGE_BDS =7.292115E-5;      //earth angular velocity (rad/s)
        //用于计算 BeiDou 卫星位置的常数
        double SIN_5 =-0.0871557427476582; //sin(-5.0 deg)
        double COS_5  =0.9961946980917456; //cos(-5.0 deg)
        ...
        if (satType != 'R') {
            ...
            switch( satType){
                case 'E' :
                    mu=MU_GAL;omge=OMGE_GAL;
                    break;
                case 'C' :
                    mu=MU_BDS;omge=OMGE_BDS;
                    break;
                default : //可选
                    mu=MU_GPS;omge=OMGE_GPS;
            }
            double satelliteClockError = computeSatelliteClockError(unixTime, eph,obsPseudorange);//计算卫星钟差
            double tGPS = computeClockCorrectedTransmissionTime(unixTime, satelliteClockError,obsPseudorange);// 返回经过卫星时钟误差校正后的信号原始发射时间
            double A = eph.getRootA() * eph.getRootA();//计算椭圆轨道的半长轴
            double tk = checkGpsTime(tGPS - eph.getToe());
            double Ek = computeEccentricAnomaly(mu,tGPS, eph);//计算偏近点角
            double fk = Math.atan2(Math.sqrt(1 - Math.pow(eph.getE(), 2)) * Math.sin(Ek), Math.cos(Ek) - eph.getE());//计算真近点角
            double phi = fk + eph.getOmega();//升交点角距
            phi = Math.IEEEremainder(phi, 2 * Math.PI);
            double u = phi + eph.getCuc() * Math.cos(2 * phi) + eph.getCus() * Math.sin(2 * phi);//摄动校正后的升交点角距
            double r = A * (1 - eph.getE() * Math.cos(Ek)) + eph.getCrc() * Math.cos(2 * phi) + eph.getCrs() * Math.sin(2 * phi);//摄动校正后的卫星矢径长度
            double ik = eph.getI0() + eph.getiDot() * tk + eph.getCic() * Math.cos(2 * phi) + eph.getCis() * Math.sin(2 * phi);//摄动校正后的轨道倾角
            //轨道平面位置
            double x1 = Math.cos(u) * r;
            double y1 = Math.sin(u) * r;
            double Omega = eph.getOmega0() + (eph.getOmegaDot() - omge) * tk - omge * eph.getToe();//升交点赤经
            ...
            //GEO卫星
            if (satType == 'C' && (eph.getSatID() <= 5 || (eph.getSatID() >= 59 && eph.getSatID() <= 61)))
            {
                //地心地固系位置
                ...
                double xg=x*coso+y*sino*COS_5+z*sino*SIN_5;
                double yg=-x*sino+y*coso*COS_5+z*coso*SIN_5;
                double zg=-y*SIN_5+z*COS_5;

                sp=new SatellitePosition(unixTime, satID, satType, xg, yg, zg);
            }
            ...
            double omegakdot = (eph.getOmegaDot() - omge);
            ...
            double tn = eph.getTauN();//钟差
            float gammaN = eph.getGammaN();//相对频率偏差
            double tk = eph.gettk();//电文帧时间
            double En = eph.getEn();//运行年龄
            double toc = eph.getToc();//钟时间(GPS周内秒)
            double toe = eph.getToe();//星历时间(GPS周内秒)
            int freqNum = eph.getfreq_num();//频率数
...
//		velDotArray.print();
    ...
    protected double computeSatelliteClockError(long unixTime, EphGps eph,double obsPseudorange) {
        if (eph.getSatType() == 'R') {   // In case of GLONASS
            ...
            //直接将导航电文中的时间定位观测瞬间的时间。观测数据的时间
            double gpsTime = (new Time(unixTime)).getGpsTime();

             //Remove signal travel time from observation time
            //从观测时间中减去信号传播时间，得到卫星信号发射时的粗略时间。
            double tRaw = (gpsTime - obsPseudorange / Constants.SPEED_OF_LIGHT);

            //没有考虑到接收信号过程中的时间损耗

            // Clock error computation
            double dt = checkGpsTime(tRaw - eph.getToe());//粗略时间与导航电文中的参考时间（Toe）之间的差值，并考虑跨周，确保GPS时间值在合理的范围内
            ...
            double gpsTime = (new Time(unixTime)).getGpsTime();//GPS时间
            //从观测时间中减去信号传播时间，得到卫星信号发射时的粗略时间。
            double tRaw = (gpsTime - obsPseudorange / Constants.SPEED_OF_LIGHT);

            //计算偏近点角（Eccentric Anomaly）
            double Ek = computeEccentricAnomaly(mu,tRaw, eph);

            //计算相对论修正项
            double dtr = -2.0 * ((Math.sqrt(mu) * eph.getRootA()) / (Constants.SPEED_OF_LIGHT * Constants.SPEED_OF_LIGHT)) * eph.getE() * Math.sin(Ek);
     ...
     */
    protected double computeEccentricAnomaly(double mu,double time, EphGps eph) {
